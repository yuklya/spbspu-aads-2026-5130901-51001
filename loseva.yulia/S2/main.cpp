...
void apply_operator(loseva::List<long long>& values, char op) {
    ...
    long long b = *values.begin(); values.pop_front();
    long long a = *values.begin(); values.pop_front();
    
    long long res = 0;
    switch (op) {
        case '+': res = safe_add(a, b); break;
        case '-': res = safe_sub(a, b); break;
        case '*': res = safe_mul(a, b); break;
        case '/': res = safe_div(a, b); break;
        case '%': res = safe_mod(a, b); break;
...
long long evaluate_expression(const std::string& line) {
    ...
    while (ss >> token) {
        ...
        if (token == "(") {
            ...
            try {
                ...
                if (idx != token.size()) throw std::runtime_error("Invalid token");
    ...
    while (!ops.empty()) {
        if (*ops.begin() == '(') throw std::runtime_error("Mismatched parentheses");
        apply_operator(values, *ops.begin());
        ops.pop_front();
    }
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "\n"; // Финальный символ переноса строки
